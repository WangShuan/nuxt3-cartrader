    ...
    <div class="flex justify-between flex-col md:flex-row">
      ...
      <div class="w-full flex flex-col text-left justify-between p-3 md:p-5">
        <div class="flex justify-between items-start">
          <h2 @click="navigateTo(`/cars/${car.id}-${car.name}`)" class="cursor-pointer text-xl font-bold hover:text-green-700 transition duration-500">{{ car.name }}
          </h2>
          <ClientOnly>
        ...
        <div class="flex justify-between items-center">
          ...
          <span class="font-bold text-lg text-green-700">NTD $ {{ car.price }} 萬</span>
        </div>
      </div>
    </div>
  </li>
