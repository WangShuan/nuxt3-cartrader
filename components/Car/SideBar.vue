<template>
  <ul class="w-full border rounded flex text-center mx-auto shadow bg-white flex-wrap">
    <li class="border-r px-4 py-2 w-full md:w-1/3 flex justify-between relative">
      <span>地點</span>
      <a @click.prevent="updateModal('location')" class="text-emerald-700" href="#">{{ route.params.city === 'All' ? 'All' : cities.find(item => item.value === route.params.city).name }}</a>
      <div v-if="modal.location" class="absolute w-[180px] border flex rounded shadow top-[100%] right-0 bg-white flex-wrap z-20">
        <span @click="onChangeLocation('All')" :class="{ 'text-emerald-700': route.params.city === 'All' }" class="cursor-pointer p-2 w-full hover:bg-gray-50 text-left">All</span>
        <span @click="onChangeLocation(city.value)" v-for="city in cities" :key="city.id" :class="{ 'text-emerald-700': route.params.city === city.value }" class="cursor-pointer p-2 w-full hover:bg-gray-50 text-left">{{ city.name }}</span>
      </div>
    </li>
    <li class="border-r px-4 py-2 w-full md:w-1/3 flex justify-between relative">
      <span>廠牌</span>
      <a @click.prevent="updateModal('brand')" class="text-emerald-700" href="#">{{ route.params.brand || 'All' }}</a>
      <div v-if="modal.brand" class="absolute w-[180px] border flex rounded shadow top-[100%] right-0 bg-white flex-wrap z-20">
        <span @click="onChangeBrand('')" :class="{ 'text-emerald-700': !route.params.brand }" class="cursor-pointer p-2 w-full hover:bg-gray-50 text-left">All</span>
        <span @click="onChangeBrand(brand)" v-for="brand in makes" :key="brand" :class="{ 'text-emerald-700': route.params.brand === brand }" class="cursor-pointer p-2 w-full hover:bg-gray-50 text-left">{{ brand }}</span>
      </div>
    </li>
    <li class="px-4 py-2 w-full md:w-1/3 flex justify-between relative">
      <span>價格</span>
      <a @click.prevent="updateModal('price')" class="text-emerald-700" href="#">{{ priceRangText }}</a>
      <div v-if="modal.price" class="absolute w-[180px] border rounded shadow p-2 top-[100%] right-0 bg-white z-20">
        ...
        <button @click="onChangePrice()" class="bg-emerald-700 w-full mt-2 p-1 text-white rounded">
          送出
        </button>
      </div>
    </li>
